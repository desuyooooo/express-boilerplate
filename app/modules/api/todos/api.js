...
router.post('/logs/insert', (req, res) => {
    db.query('INSERT INTO logs(`todo_id`, `mode`, `modified_by`) VALUES (?, ?, ?)', [req.body.todoid, req.body.mode, req.body.modified_by], (err, results, fields) => {
        if (err) return res.status(400).send({ error: err.toString() });
...
table name: logs
create table logs (id int primary key not null auto_increment, todo_id int not null, mode ENUM('add', 'update', 'delete', 'checked', 'comment'), modified_by int not null, date_modified datetime default CURRENT_TIMESTAMP());
*/
